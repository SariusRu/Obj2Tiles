...
public class B3dmHeader
{
    ...
    public int Length => 28 + FeatureTableJsonByteLength + FeatureTableBinaryByteLength + BatchTableJsonByteLength +
    ...
    public byte[] AsBinary()
    {
        ...
        return magicBytes.Concat(versionBytes).Concat(byteLengthBytes).Concat(featureTableJsonByteLengthBytes)
            .Concat(featureTableBinaryByteLengthBytes).Concat(batchTableJsonByteLength)
            .Concat(batchTableBinaryByteLength).ToArray();
    }
    ...
    public List<string> Validate()
    {
        ...
        if (batchTableBinaryByteOffset % 8 > 0) res.Add("Batch table binary must be aligned to an 8-byte boundary.");
        if (glbByteOffset % 8 > 0) res.Add("Glb must be aligned to an 8-byte boundary.");
