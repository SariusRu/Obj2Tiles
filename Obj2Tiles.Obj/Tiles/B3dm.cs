...
public class B3dm
{
    ...
    public byte[] ToBytes()
    {
        ...
        B3dmHeader.ByteLength = GlbData.Length + header_length + featureTableJson.Length +
                                Encoding.UTF8.GetByteCount(batchTableJson) + batchTableBinary.Length +
        ...
        if (featureTableBinary != null) binaryWriter.Write(featureTableBinary);
        binaryWriter.Write(Encoding.UTF8.GetBytes(batchTableJson));
        if (batchTableBinary != null) binaryWriter.Write(batchTableBinary);
